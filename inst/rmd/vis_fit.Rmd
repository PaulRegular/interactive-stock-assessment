...
cols <-  viridis::viridis(4)[c(2, 3, 1, 4)]
...
plot_obs_pred(data = d, showlegend = TRUE, 
              type = "log", group = "age",
              group_cols = viridis::viridis(nlevels(d$age), 
                                            direction = 1, end = 0.8),
              ylab = "Numbers")
```
...
```{r}
plot_obs_pred(data = d, showlegend = TRUE, 
              type = "log", group = "cohort",
              group_cols = viridis::viridis(nlevels(d$cohort), 
                                            direction = 1, end = 0.8),
              ylab = "Numbers")
```
...
```{r}
plot_obs_pred(data = d, showlegend = TRUE, 
              type = "linear", group = "age",
              group_cols = viridis::viridis(nlevels(d$age), 
                                            direction = 1, end = 0.8),
              ylab = "Proportion", stack_group = TRUE)
```

> Note: values are stacked

### Fits by cohort

```{r}
plot_obs_pred(data = d, showlegend = TRUE, 
              type = "linear", group = "cohort",
              group_cols = viridis::viridis(nlevels(d$cohort), 
                                            direction = 1, end = 0.8),
              ylab = "Proportion", stack_group = TRUE)
```
...
base <- plot_ly(d, symbol = ~period, colors = cols[1:2], symbols = c(16, 1)) %>% 
  group_by(exp) %>% 
...
sum_d %>%
  group_by(exp) %>%
  plot_ly(x = ~cap_year, frame = ~exp, hoverinfo = "x+y+text",
          showlegend = FALSE) %>%
  add_markers(y = ~sqrt(sum_obs), color = I(cols[1]),
              text = ~paste("exp:", exp, "</br>", "observed N:", sum_obs)) %>%
  add_lines(y = ~sqrt(sum_pred), color = I(cols[2]),
            text = ~paste("exp:", exp, "</br>", "predicted N:", round(sum_pred))) %>%
...
bms_plot <- trans_est(retro_tabs$log_ssb) %>%
  plot_mohns(col = cols[1], ylab = "SSB")

abn_plot <- trans_est(retro_tabs$log_recruitment, scale = 0.001) %>%
  plot_mohns(ylab = "Recruitment (age 2)", col = cols[1])

blim_plot <- trans_est(retro_tabs$log_ssb_lrp) %>%
  plot_mohns(ylab = "SSB/Blim", col = cols[1])
...
bms_plot <- retro_tabs$log_ssb %>%
  plot_retro_deltas(in_col = cols[1], ylab = "log(SSB)")

abn_plot <- retro_tabs$log_recruitment %>%
  plot_retro_deltas(ylab = "log(Recruitment)", in_col = cols[1])

blim_plot <- retro_tabs$log_ssb_lrp %>%
  plot_retro_deltas(ylab = "log(SSB/Blim)", in_col = cols[1])
...
z_plot <- trans_est(retro_tabs$log_aveZ) %>%
  plot_mohns(ylab = "Average Z (ages 5+)", col = cols[1])

m_plot <- trans_est(retro_tabs$log_aveM) %>%
  plot_mohns(ylab = "Average M (ages 5+)", col = cols[1])

f_plot <- trans_est(retro_tabs$log_aveF) %>%
  plot_mohns(ylab = "Average F (ages 5+)", col = cols[1])
...
z_plot <- retro_tabs$log_aveZ %>%
  plot_retro_deltas(ylab = "log(Average Z)", in_col = cols[1])

m_plot <- retro_tabs$log_aveM %>%
  plot_retro_deltas(ylab = "log(Average M)", in_col = cols[1])

f_plot <- retro_tabs$log_aveF %>%
  plot_retro_deltas(ylab = "log(Average F)", in_col = cols[1])
...
plot_proj(data = d, xlim = xlim, ylim = ylim, ylab = "SSB/Blim",
          vline = max(years) + 1, hline = 1, col = cols[1], darken_proj = TRUE)
