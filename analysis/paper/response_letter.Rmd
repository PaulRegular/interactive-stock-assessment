...
*This is a very good point and, in hindsight, we see why the paper leaves this impression. To try and rectify this, we have clarified the intent of interactive tools in several places in the paper. We now explicitly note in the introduction that these tools are not meant to replace static modes of communication, rather they are often intended to supplement standard documents. This point is reiterated in the  "Towards open stock assessment" section, where we also highlight an example from Appendix A to make the point that advisory documents (i.e. a linear story) can be interactive. As with the previous iteration, we contend that a non-linear format can be useful for evaluating a stock assessment model. That being said, we take your point that a story may be more suitable in some cases. This format is possible with interactive documents as well because dashboards can be nested within html slide-shows, or dashboards can include "story-boards". These are very flexible tools and in this iteration of the manuscript we hope we have clarified their flexibility without discounting static or linear formats. In short, we want to make the point that interactive tools are a potentially powerful tool to add to a biologists’ toolkit.*
...
*See "stand alone" comment above. Nevertheless, we have added equations and an additional figure to provide more context. We have also added more text and equations to subsequent tabs.*
...
*More references have been added (all of the key references for the accepted assessment model).*
...
*Agreed, the colors were not ideal. We have made the colour scheme more consistent throughout the tool.*
...
*True, the fits by age plots were confusing and only useful if the user selected a specific age using the legend. These plots have been removed and replaced by plots with a slider for selecting specific ages. Now the facet plot allows a user to assess the fits at a glance with one plot and, if interested in the details, shift to the frame plot to take a closer look at the fits to specific ages.*
...
*All excellent suggestions, which will improve future iterations of the tools. These two tools, unlike the NCAM dashboard, were mainly built for use by the developer themselves. When these tools move to being used by larger communities, further documentation will be essential. Our intent in showing these two ‘in-house’ tools was to demonstrate that interactive visualization tools can be a valuable addition to an analysts tool box.*
...
*We have expanded the "Torwards open stock-assessment" section and, in the process, we hope we have clarified how these tools improve accessability and transparency by making it easy for others to access the details.*
...
*This is a very astute observation and it is most likely related to changes in how Plaice length frequencies were recorded (3 to 1 cm scale). While this is an important detail to be cognisant of when analyzing these data, we have opted not to get into the minutiae of the sampling as it is outside the scope of this paper.*
